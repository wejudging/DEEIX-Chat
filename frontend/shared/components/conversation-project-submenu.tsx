...
type ConversationProjectMenuItemsProps = {
...
export function ConversationProjectMenuItems({
  unassignedLabel,
  currentProjectID,
  selectionKnown = true,
  projects,
  onSelect,
}: ConversationProjectMenuItemsProps) {
  return (
    <>
      <DropdownMenuItem
      ...
      >
        ...
        <Check
          ...
          className={cn(
            "size-3.5 shrink-0",
            selectionKnown && !currentProjectID ? "text-foreground" : "opacity-0",
          )}
          strokeWidth={1.7}
        />
      </DropdownMenuItem>
        ...
        return (
          ...
          >
            ...
            <Check
              ...
              className={cn(
                "size-3.5 shrink-0",
                selectionKnown && selected ? "text-foreground" : "opacity-0",
              )}
              strokeWidth={1.7}
            />
          </DropdownMenuItem>
...
type ConversationProjectSubmenuProps = ConversationProjectMenuItemsProps & {
  label: string;
};
  ...
  return (
    <DropdownMenuSub>
      ...
      <DropdownMenuSubContent className="max-h-[min(18rem,var(--radix-dropdown-menu-content-available-height))] min-w-44 overflow-y-auto p-1.5">
        <ConversationProjectMenuItems {...menuProps} />
      </DropdownMenuSubContent>
    </DropdownMenuSub>
  );
}
