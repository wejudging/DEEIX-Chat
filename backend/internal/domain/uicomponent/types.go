...
var builtinSpecs = []builtinSpec{
	...
	},
	{
		...
		Rules:        "scatter 的 x 为数值；series.type 让单条序列改画柱/线/面积做组合图，series.axis: right 用右侧第二 Y 轴；stacked 堆叠，percent 配合 stacked 归一到 100%",
	...
	},
	{
		...
		Rules:        "expr 可以是关于 x 的表达式、y = …、x = …（关于 y）或含 x、y 的方程（如 x^2 + y^2 = 4）；乘法必须写 *；支持 sin cos tan asin acos atan sinh cosh tanh sqrt cbrt abs exp ln log log2 log10 pow floor ceil round sign min max 与常量 pi e",
	...
	},
	{
		...
		Rules:        "delta 是变化量文本，trend 决定颜色；history 从旧到新，periods 是各点标签",
	...
	},
	{
		...
		Rules:        "日期用 YYYY-MM-DD；end 与 days 二选一；depends 引用其它任务的 id（未给 id 时用 name）；milestone 画成菱形",
	...
	},
	{
		...
		Rules:        "expr 只能用 + - * / ^ %、括号、输入 key 与 abs sqrt pow min max round floor ceil log exp；highlight 标出主结果",
	...
	},
	{
		...
		Rules:        "answer 是正确选项下标，不给 answer 的题只用于分流不计分；选项可写成 {text, next, explanation} 跳到指定 id 的题，next: \"end\" 结束；题目级 next 是默认下一题",
